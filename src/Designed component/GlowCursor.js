...
const GlowCursor = ({
  ...
  idleTimeout = 700,
  fadeDuration = 900,
  blendMode = 'normal', // default changed: avoids CSS mix-blend-mode compositor cost
  ...
  useEffect(() => {
    ...
    // --- Cached rect: avoids forced layout reads on every pointermove ---
    let rectCache = { left: 0, top: 0, width: 1, height: 1 };
    ...
    const updatePointer = event => {
      // No getBoundingClientRect() here anymore — uses the cached rect,
      // so mousemove never forces a synchronous layout flush mid-scroll.
      const x = clamp(event.clientX - rectCache.left, 0, rectCache.width);
      const y = clamp(rectCache.height - (event.clientY - rectCache.top), 0, rectCache.height);
    ...
    const render = now => {
      if (destroyed || !isVisible) {
        raf = 0; // stop scheduling entirely instead of looping while hidden
        return;
      }
    ...
    // --- Only run the loop while the component is actually on screen ---
    const visibilityObserver = new IntersectionObserver(
      ([entry]) => {
        isVisible = entry.isIntersecting;
        if (isVisible && !raf && !destroyed) {
  ...
  return (
    <div ref={containerRef} className={`glow-cursor${className ? ` ${className}` : ''}`} style={{ position: 'relative', ...style }} {...rest}>
      <canvas
        ref={canvasRef}
        className="glow-cursor__canvas"
        style={{ position: 'absolute', inset: 0, mixBlendMode: blendMode, pointerEvents: 'none' }}
        aria-hidden="true"
      />
